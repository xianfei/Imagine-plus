export default {
  about: 'About {0}',
  file: 'File',
  open: 'Open...',
  add: 'Add',
  save: 'Save',
  save_cover: 'Save and overwrite',
  save_new: 'Save with new name auto',
  save_dir: 'Export to',
  save_as: 'Save as',
  save_success: 'Save successfully',
  clear: 'Clear all',
  clear_increased: 'Clear increased files',
  choose_images: 'Choose images',
  drag_files: 'Drag pictures here from the local folder',
  colors: 'Colors',
  quality: 'Quality',
  visit: 'Visit homepage',
  ok: 'OK',
  before_optimized: 'Before',
  after_optimized: 'After',
  compare: 'Compare',
  new_version: 'Upgrade',
  apply_now: 'Apply now',
  resize: 'Resize',
  resize_long_edge: 'Max long edge',
  resize_short_edge: 'Max short edge',
  resize_scale: 'Scale',
  resize_apply: 'Apply to all',
  resize_clear: 'Clear resize',
  settings: 'Settings',
  format_mapping: 'Format conversion',
  format_mapping_desc: 'Default output format for each input format',
  output_options: 'Output options',
  output_options_desc: 'Encoding parameters for each output format',
  general: 'General',
  keep_metadata: 'Keep metadata (EXIF / ICC)',
  progressive_enc: 'Progressive JPEG / interlaced PNG',
  check_update: 'Check for updates at launch',
  language: 'Language',
  language_auto: 'Auto (system)',
  concurrency: 'Parallel tasks',
  concurrency_auto: 'Auto ({0})',
  apply_existing: 'Re-optimize existing tasks with these defaults',
}
